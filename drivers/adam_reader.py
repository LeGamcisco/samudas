...
load_dotenv("C:/xampp/htdocs/trudas/.env")
...
def get_sensors():
    try:
        ...
        sql = "SELECT is_has_reference,sensors.id,sensors.stack_id,sensors.name,extra_parameter,o2_correction,formula,analyzer_ip,port,stacks.oxygen_reference FROM sensors LEFT JOIN stacks ON sensors.stack_id = stacks.id where sensors.deleted_at is null ORDER BY id"
        cursor.execute(sql)
...
def get_config():
    try:
        ...
        print("Connection Error: ",e)
        return None
def get_references(sensor_id):
    try:
        conn = connect_db()
        cursor = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        sql = "SELECT * FROM \"references\" WHERE sensor_id={}".format(sensor_id)
        cursor.execute(sql)
        result = cursor.fetchall()
        return result
    except Exception as e:
